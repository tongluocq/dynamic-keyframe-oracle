...
${this.generateDynamicExampleCaseSection()}
...
`;
  ...
  /**
   * Generate dynamic section for Example&Case report from actual operation results
   */
  private generateDynamicExampleCaseSection(): string {
    const results = this.getResults();
    const interventions = this.getResultsByType('intervention') as InterventionOperationResult[];
    const counterfactuals = this.getResultsByType('counterfactual') as CounterfactualOperationResult[];
    const prescriptives = this.getResultsByType('prescriptive') as PrescriptiveOperationResult[];
    const inferences = this.getResultsByType('cvgg_inference') as CVGGInferenceResult[];

    if (interventions.length === 0 && counterfactuals.length === 0 && prescriptives.length === 0 && inferences.length === 0) {
      return '';
    }

    let section = `## 8. 🔄 Dynamic Operation Results (${results.length} total operations)\n\n`;
    section += `> The following data was captured from actual IMSCHM session operations, complementing the reference examples above.\n\n`;

    if (inferences.length > 0) {
      section += `### 8.1 CVGG Inference Results (${inferences.length} runs)\n\n`;
      section += `| # | Classification | Confidence | ATE | CATE | DE | IE | Anomaly |\n`;
      section += `|---|---------------|------------|-----|------|----|----|--------|\n`;
      inferences.slice(0, 15).forEach((inf, i) => {
        const d = inf.data;
        section += `| ${i + 1} | ${d.classification.className} | ${(d.classification.confidence * 100).toFixed(1)}% | ${d.causalEffects.ATE.toFixed(4)} | ${d.causalEffects.CATE.toFixed(4)} | ${d.causalEffects.directEffect.toFixed(4)} | ${d.causalEffects.indirectEffect.toFixed(4)} | ${(d.anomalyScore * 100).toFixed(1)}% |\n`;
      });
      section += '\n';
    }

    if (interventions.length > 0) {
      section += `### 8.2 do-Calculus Intervention Results (${interventions.length} executions)\n\n`;
      section += `| # | Intervention | Variable | Target | Primary | Total | Pre-Risk | Post-Risk | Risk Δ |\n`;
      section += `|---|-------------|----------|--------|---------|-------|----------|-----------|--------|\n`;
    ...
    if (counterfactuals.length > 0) {
      section += `### 8.3 Counterfactual What-If Results (${counterfactuals.length} queries)\n\n`;
      section += `| # | Query | Baseline | Counterfactual | Effect | Confidence | Risk |\n`;
      section += `|---|-------|----------|---------------|--------|------------|------|\n`;
      counterfactuals.slice(0, 15).forEach((cf, i) => {
        const d = cf.data;
        section += `| ${i + 1} | ${d.query.description.substring(0, 40)} | ${(d.baselineOutcome * 100).toFixed(1)}% | ${(d.counterfactualOutcome * 100).toFixed(1)}% | ${(d.causalEffect * 100).toFixed(1)}% | ${(d.confidence * 100).toFixed(1)}% | ${d.riskChange} |\n`;
      });
      section += '\n';
    }

    if (prescriptives.length > 0) {
      section += `### 8.4 Prescriptive AI Results (${prescriptives.length} analyses)\n\n`;
      section += `| # | Health | Risk Level | Recs | Top Action | Priority | Risk Reduction |\n`;
      section += `|---|--------|------------|------|------------|----------|----------------|\n`;
      prescriptives.slice(0, 10).forEach((p, i) => {
        const d = p.data;
        const top = d.topPriority || d.recommendations[0];
        section += `| ${i + 1} | ${d.systemHealthScore.toFixed(0)}/100 | ${d.riskLevel.toUpperCase()} | ${d.recommendations.length} | ${top?.title?.substring(0, 25) || '—'} | ${top?.priority?.toUpperCase() || '—'} | ${top ? (top.estimatedImpact.riskReduction * 100).toFixed(0) + '%' : '—'} |\n`;
      });
      section += '\n';
    }
